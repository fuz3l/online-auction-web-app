...
const DetailsPage = () => {
  ...
  return (
    <>
      ...
      <div className="h-screen flex justify-center items-center bg-gray-100 text-gray-900">
        <div className="container h-screen flex justify-center items-center mx-auto">
          <div className="grid md:grid-cols-2 p-12 p gap-8 bg-white shadow-lg rounded-xl ">
            <div>
            ...
            <div className="bg-gray-50 p-6 rounded-lg shadow-md">
              ...
              {bids.length > 0 ? (
                <ul className="mt-3 space-y-2">
                  {bids.map((bid, index) => (
                    <li key={index} className=" border mb-2 rounded-lg shadow-sm bg-white">
                      <strong>{bid.bidderName}</strong>: ₹{bid.amount} - {bid.timestamp?.toDate ? bid.timestamp.toDate().toLocaleString() : "Just now"}
                    </li>
                  ))}
                </ul>
              ...
              <button
                onClick={deleteItem}
                className="w-full bg-red-600 text-white py-3 rounded-lg text-lg mt-4 hover:bg-red-700 transition duration-200"
              >
                Delete Item
              </button>
            </div>
